...
let currentLang = 'ja';
// Check URL for ?lang=en so pages can be opened in English when linked from English pages
...
function updateText() {
    ...
    // --- Update anchor hrefs for English <-> Japanese navigation when available ---
    document.querySelectorAll('a[data-en-href]').forEach(a => {
        if (currentLang === 'en') {
            a.href = a.getAttribute('data-en-href');
        } else {
            a.href = a.getAttribute('data-ja-href');
        }
    });
}